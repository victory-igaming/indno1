...
const queryCat = qs.stringify(
  {
    pagination: {
      limit: 30, // load more games so slider feels rich
    },
    populate: {
      gameicon: { populate: "*" },
    },
...
export default function TrandingGame() {
  ...
  const liveCasinoList = useMemo(() => {
    if (!data) return [];
    return [...data]; // keep original untouched
  }, [data]);
    ...
    <section className="tg-section">
      <div className="tg-header">
        <div className="tg-title-wrap">
          ...
          <h3 className="tg-title">Trending Games</h3>
        </div>
      ...
      <div className="tg-slider" ref={sliderRef}>
        {liveCasinoList.map((game: any) => {
          const imageUrl = getStrapiMedia(game.gameicon?.url);

          return (
            <div key={game.id} className="tg-card">
              <Link className="tg-card-link" href={`/playgame/${game.seourl}`}>
                {imageUrl ? (
                  <img
                    src={imageUrl}
